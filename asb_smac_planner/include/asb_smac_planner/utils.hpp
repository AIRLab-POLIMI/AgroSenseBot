...
#include "asb_smac_planner/types.hpp"
...
*/
inline geometry_msgs::msg::Pose getWorldCoords(const float &mx, const float &my, const nav2_costmap_2d::Costmap2D *costmap) {
    geometry_msgs::msg::Pose msg;
    msg.position.x = static_cast<float>(costmap->getOriginX()) + (mx - 0.5) * costmap->getResolution();
    msg.position.y = static_cast<float>(costmap->getOriginY()) + (my - 0.5) * costmap->getResolution();
    return msg;
}
...
inline double findCircumscribedCost(std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap) {
    ...
    for (layer = costmap->getLayeredCostmap()->getPlugins()->begin(); layer != costmap->getLayeredCostmap()->getPlugins()->end(); ++layer) {
    ...
    if (!inflation_layer_found) {
        RCLCPP_WARN(rclcpp::get_logger("computeCircumscribedCost"), "No inflation layer found in costmap configuration. "
 ...
 */
inline void fromJsonToMotionPrimitive(const nlohmann::json &json, MotionPrimitive &motion_primitive) {
 ...
 */
inline std::vector<geometry_msgs::msg::Point> transformFootprintToEdges(const geometry_msgs::msg::Pose &pose, const std::vector<geometry_msgs::msg::Point> &footprint) {
 ...
 */
inline visualization_msgs::msg::Marker createMarker(const std::vector<geometry_msgs::msg::Point> edge, double line_size, unsigned int i, const std::string &frame_id, const rclcpp::Time &timestamp) {
 ...
 * @return marker populated
 */
inline void fillExpansionsMarker(visualization_msgs::msg::Marker &m, const std::string &ns, double line_size, float r, float g, float b, float a, const std::string &frame_id, const rclcpp::Time &timestamp) {
