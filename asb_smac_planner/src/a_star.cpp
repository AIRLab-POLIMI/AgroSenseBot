...
#include "asb_smac_planner/a_star.hpp"
...
namespace asb_smac_planner {
  ...
  template<typename NodeT>
  void AStarAlgorithm<NodeT>::initialize(const bool &allow_unknown, int &max_iterations, const int &max_on_approach_iterations, const double &max_planning_time, const float &lookup_table_size, const unsigned int &dim_3_size) {
  ...
  template<>
  void AStarAlgorithm<Node2D>::initialize(const bool &allow_unknown, int &max_iterations, const int &max_on_approach_iterations, const double &max_planning_time, const float & /*lookup_table_size*/, const unsigned int &dim_3_size) {
      _traverse_unknown = allow_unknown;
      _max_iterations = max_iterations;
      _max_on_approach_iterations = max_on_approach_iterations;
      _max_planning_time = max_planning_time;
      ...
      _dim3_size = dim_3_size;
      _expander = std::make_unique<AnalyticExpansion<Node2D>>(_motion_model, _search_info, _traverse_unknown, _dim3_size);
  }
  ...
  template<typename NodeT>
  typename AStarAlgorithm<NodeT>::NodePtr AStarAlgorithm<NodeT>::addToGraph(const unsigned int &index) {
  ...
  template<>
  void AStarAlgorithm<Node2D>::setStart(const unsigned int &mx, const unsigned int &my, const unsigned int &dim_3) {
      if (dim_3 != 0) {
  ...
  template<typename NodeT>
  void AStarAlgorithm<NodeT>::setStart(const unsigned int &mx, const unsigned int &my, const unsigned int &dim_3) {
      _start = addToGraph(NodeT::getIndex(mx, my, dim_3));
      _start->setPose(Coordinates(static_cast<float>(mx), static_cast<float>(my), static_cast<float>(dim_3)));
  }

  template<>
  void AStarAlgorithm<Node2D>::setGoal(const unsigned int &mx, const unsigned int &my, const unsigned int &dim_3) {
      if (dim_3 != 0) {
  ...
  template<typename NodeT>
  void AStarAlgorithm<NodeT>::setGoal(const unsigned int &mx, const unsigned int &my, const unsigned int &dim_3) {
      _goal = addToGraph(NodeT::getIndex(mx, my, dim_3));

      typename NodeT::Coordinates goal_coords(static_cast<float>(mx), static_cast<float>(my), static_cast<float>(dim_3));
  ...
  template<typename NodeT>
  bool AStarAlgorithm<NodeT>::createPath(CoordinateVector &path, int &iterations, const float &tolerance, std::vector<std::tuple<float, float, float, float, float, bool>> *expansions_log) {
      ...
      while (iterations < getMaxIterations() && !_queue.empty()) {
          ...
          // Check for planning timeout only on every N-th iteration
          ...
          // 1) Pick N_best from the open set s.t. min(f(N_best)), remove from queue
          ...
          // 2) Mark N_best as visited
          current_node->visited();

          // 2.1) Use an analytic expansion (if available) to generate a path
          expansion_result = nullptr;
          expansion_result = _expander->tryAnalyticExpansion(current_node, getGoal(), neighborGetter, analytic_iterations, closest_distance);
          if (expansion_result != nullptr) {

              // Save current edge coordinates for debug
              if (expansions_log) {
                  float current_node_x, current_node_y;
                  current_node->getPose(current_node_x, current_node_y);
                  float expansion_result_node_x, expansion_result_node_y;
                  expansion_result->getPose(expansion_result_node_x, expansion_result_node_y);
                  auto edge_info = std::make_tuple(
                        current_node_x,
                        current_node_y,
                        expansion_result_node_x,
                        expansion_result_node_y,
                        0.0,
                        true
                  );

                  expansions_log->push_back(edge_info);
              }
          ...
          // 4) Expand neighbors of N_best not visited
          neighbors.clear();
          ...
          for (neighbor_iterator = neighbors.begin(); neighbor_iterator != neighbors.end(); ++neighbor_iterator) {
              ...
              if (g_cost < neighbor->getAccumulatedCost()) {
                  ...
                  if (expansions_log) {
                      float current_node_x, current_node_y;
                      current_node->getPose(current_node_x, current_node_y);
                      float neighbor_node_x, neighbor_node_y;
                      neighbor->getPose(neighbor_node_x, neighbor_node_y);
                      auto edge_info = std::make_tuple(
                            current_node_x,
                            current_node_y,
                            neighbor_node_x,
...
// Instantiate algorithm for the supported template types
  template
  class AStarAlgorithm<Node2D>;

  template
  class AStarAlgorithm<NodeHybrid>;

  template
  class AStarAlgorithm<NodeLattice>;
