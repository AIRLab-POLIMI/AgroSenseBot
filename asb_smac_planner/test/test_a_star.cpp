...
#include "asb_smac_planner/node_lattice.hpp"
...
TEST(AStarTest, test_a_star_2d)
{
  ...
  asb_smac_planner::AStarAlgorithm<asb_smac_planner::Node2D> a_star(
    asb_smac_planner::MotionModel::TWOD, info);
  ...
  std::unique_ptr<asb_smac_planner::GridCollisionChecker> checker =
    std::make_unique<asb_smac_planner::GridCollisionChecker>(costmapA, 1, lnode);
  checker->setFootprint(nav2_costmap_2d::Footprint(), true, 0.0);
  a_star.setCollisionChecker(checker.get());
  a_star.setStart(20u, 20u, 0);
  a_star.setGoal(80u, 80u, 0);
  asb_smac_planner::Node2D::CoordinateVector path;
  ...
  asb_smac_planner::AStarAlgorithm<asb_smac_planner::Node2D> a_star_2(
    asb_smac_planner::MotionModel::TWOD, info);
...
TEST(AStarTest, test_a_star_se2)
{
  ...
  asb_smac_planner::AStarAlgorithm<asb_smac_planner::NodeHybrid> a_star(
    asb_smac_planner::MotionModel::DUBIN, info);
  int max_iterations = 10000;
  ...
  std::unique_ptr<asb_smac_planner::GridCollisionChecker> checker =
    std::make_unique<asb_smac_planner::GridCollisionChecker>(costmapA, size_theta, lnode);
  checker->setFootprint(nav2_costmap_2d::Footprint(), true, 0.0);
  ...
  asb_smac_planner::NodeHybrid::CoordinateVector path;
  EXPECT_TRUE(a_star.createPath(path, num_it, tolerance));
...
TEST(AStarTest, test_a_star_lattice)
{
  ...
  info.lattice_filepath =
    ament_index_cpp::get_package_share_directory("asb_smac_planner") +
  ...
  asb_smac_planner::AStarAlgorithm<asb_smac_planner::NodeLattice> a_star(
    asb_smac_planner::MotionModel::STATE_LATTICE, info);
  int max_iterations = 10000;
  float tolerance = 10.0;
  int it_on_approach = 10;
  double max_planning_time = 120.0;
  ...
  asb_smac_planner::NodeLattice::CoordinateVector path;
  EXPECT_TRUE(a_star.createPath(path, num_it, tolerance));
...
{
  asb_smac_planner::MotionModel mm = asb_smac_planner::MotionModel::UNKNOWN;  // unknown
  EXPECT_EQ(asb_smac_planner::toString(mm), std::string("Unknown"));
  mm = asb_smac_planner::MotionModel::TWOD;  // 2d
  EXPECT_EQ(asb_smac_planner::toString(mm), std::string("2D"));
  mm = asb_smac_planner::MotionModel::DUBIN;  // dubin
  EXPECT_EQ(asb_smac_planner::toString(mm), std::string("Dubin"));
  mm = asb_smac_planner::MotionModel::REEDS_SHEPP;  // reeds-shepp
  EXPECT_EQ(asb_smac_planner::toString(mm), std::string("Reeds-Shepp"));

  EXPECT_EQ(
    asb_smac_planner::fromString(
      "2D"), asb_smac_planner::MotionModel::TWOD);
  EXPECT_EQ(asb_smac_planner::fromString("DUBIN"), asb_smac_planner::MotionModel::DUBIN);
  EXPECT_EQ(
    asb_smac_planner::fromString(
      "REEDS_SHEPP"), asb_smac_planner::MotionModel::REEDS_SHEPP);
  EXPECT_EQ(asb_smac_planner::fromString("NONE"), asb_smac_planner::MotionModel::UNKNOWN);
}
