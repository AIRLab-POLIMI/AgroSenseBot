...
#include "asb_smac_planner/smoother.hpp"
#include "ament_index_cpp/get_package_share_directory.hpp"

using namespace asb_smac_planner;  // NOLINT
...
class SmootherWrapper : public asb_smac_planner::Smoother
{
public:
  explicit SmootherWrapper(const SmootherParams & params)
  : asb_smac_planner::Smoother(params)
  {}
...
TEST(SmootherTest, test_full_smoother)
{
  ...
  asb_smac_planner::SmootherParams params;
  ...
  asb_smac_planner::AStarAlgorithm<asb_smac_planner::NodeHybrid> a_star(
    asb_smac_planner::MotionModel::REEDS_SHEPP, info);
  ...
  std::unique_ptr<asb_smac_planner::GridCollisionChecker> checker =
    std::make_unique<asb_smac_planner::GridCollisionChecker>(costmap, size_theta, node);
  ...
  for (int i = path.size() - 1; i >= 0; --i) {
    pose.pose = asb_smac_planner::getWorldCoords(path[i].x, path[i].y, costmap);
    pose.pose.orientation = asb_smac_planner::getWorldOrientation(path[i].theta);
