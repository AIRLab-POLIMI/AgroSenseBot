...
{
  std::string pkg_share_dir = ament_index_cpp::get_package_share_directory("asb_smac_planner");
  std::string filePath =
    pkg_share_dir +
  ...
  asb_smac_planner::LatticeMetadata metaData;
  asb_smac_planner::MotionPrimitive myPrimitive;
  asb_smac_planner::MotionPose pose;

  json jsonMetaData = j["lattice_metadata"];
  json jsonPrimatives = j["primitives"];
  json jsonPose = jsonPrimatives[0]["poses"][0];

  asb_smac_planner::fromJsonToMetaData(jsonMetaData, metaData);
  ...
  std::vector<asb_smac_planner::MotionPrimitive> myPrimitives;
  for (unsigned int i = 0; i < jsonPrimatives.size(); ++i) {
    asb_smac_planner::MotionPrimitive newPrimative;
    asb_smac_planner::fromJsonToMotionPrimitive(jsonPrimatives[i], newPrimative);
...
TEST(NodeLatticeTest, test_node_lattice_neighbors_and_parsing)
{
  ...
  asb_smac_planner::NodeLattice::initMotionModel(
    asb_smac_planner::MotionModel::STATE_LATTICE, x, y, angle_quantization, info);

  asb_smac_planner::NodeLattice aNode(0);
  aNode.setPose(asb_smac_planner::NodeHybrid::Coordinates(0, 0, 0));
  asb_smac_planner::MotionPrimitivePtrs projections =
    asb_smac_planner::NodeLattice::motion_table.getMotionPrimitives(&aNode);
  ...
  EXPECT_NEAR(
    asb_smac_planner::NodeLattice::motion_table.getLatticeMetadata(
...
TEST(NodeLatticeTest, test_node_lattice)
{
  ...
  // Check defaults
  asb_smac_planner::NodeLattice aNode(0);
  asb_smac_planner::NodeLattice testA(49);
  ...
  asb_smac_planner::NodeLattice testC(49);
  ...
  testC.setPose(asb_smac_planner::NodeLattice::Coordinates(10.0, 5.0, 4));
...
TEST(NodeLatticeTest, test_get_neighbors)
{
  ...
  asb_smac_planner::NodeLattice node(49);
  ...
  std::function<bool(const unsigned int &, asb_smac_planner::NodeLattice * &)> neighborGetter =
    [&, this](const unsigned int & index, asb_smac_planner::NodeLattice * & neighbor_rtn) -> bool
    {
      // because we don't return a real object
      return false;
    };

  asb_smac_planner::NodeLattice::NodeVector neighbors;
...
TEST(NodeLatticeTest, test_node_lattice_custom_footprint)
{
  ...
  std::unique_ptr<asb_smac_planner::GridCollisionChecker> checker =
    std::make_unique<asb_smac_planner::GridCollisionChecker>(costmap, 72, lnode);
  ...
  asb_smac_planner::MotionPrimitivePtrs motion_primitives =
    asb_smac_planner::NodeLattice::motion_table.getMotionPrimitives(&node);
