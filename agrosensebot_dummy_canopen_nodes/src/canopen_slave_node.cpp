...
void CANOpenSlaveNode::send_TPDO_1(int16_t FAN_controller_temperature, int16_t FAN_motor_temperature, int16_t FAN_motor_RPM, int16_t FAN_battery_current_display) {
  RCLCPP_INFO(ros2_bridge_node_->get_logger(), "send_TPDO_1: writing 0x%X to 0x%X:%X", FAN_controller_temperature, IDX_MOTOR_DRIVE_DATA, SUB_IDX_FAN_controller_temperature);
  (*this)[IDX_MOTOR_DRIVE_DATA][SUB_IDX_FAN_controller_temperature] = FAN_controller_temperature;

  RCLCPP_INFO(ros2_bridge_node_->get_logger(), "send_TPDO_1: writing 0x%X to 0x%X:%X", FAN_motor_temperature, IDX_MOTOR_DRIVE_DATA, SUB_IDX_FAN_motor_temperature);
  (*this)[IDX_MOTOR_DRIVE_DATA][SUB_IDX_FAN_motor_temperature] = FAN_motor_temperature;

  RCLCPP_INFO(ros2_bridge_node_->get_logger(), "send_TPDO_1: writing 0x%X to 0x%X:%X", FAN_motor_RPM, IDX_MOTOR_DRIVE_DATA, SUB_IDX_FAN_motor_RPM);
  (*this)[IDX_MOTOR_DRIVE_DATA][SUB_IDX_FAN_motor_RPM] = FAN_motor_RPM;

  int16_t FAN_motor_RPM_read = (*this)[IDX_MOTOR_DRIVE_DATA][SUB_IDX_FAN_motor_RPM];
  RCLCPP_INFO(ros2_bridge_node_->get_logger(), "                     0x%X", FAN_motor_RPM_read);

  RCLCPP_INFO(ros2_bridge_node_->get_logger(), "send_TPDO_1: writing 0x%X to 0x%X:%X", FAN_battery_current_display, IDX_MOTOR_DRIVE_DATA, SUB_IDX_FAN_battery_current_display);
  (*this)[IDX_MOTOR_DRIVE_DATA][SUB_IDX_FAN_battery_current_display] = FAN_battery_current_display;
