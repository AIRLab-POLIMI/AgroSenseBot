...
geometry_msgs::msg::TwistStamped RegulatedPurePursuitController::computeVelocityCommands(const geometry_msgs::msg::PoseStamped &pose, const geometry_msgs::msg::Twist &speed, nav2_core::GoalChecker *goal_checker) {
    ...
    // If we already passed the goal but the goal is not considered reached (by the goal checker),
    // we are stuck and should throw an exception to stop the control early (before the progress checker does)
    if(transformed_plan.poses.size() == 1) {  // if we reached the end of the plan
        double theta = tf2::getYaw(goal_pose.pose.orientation);  // goal angle in robot frame, -PI < theta < PI
        if(theta > M_PI) theta -= 2 * M_PI;

        bool goal_direction_is_forward = std::fabs(theta) < M_PI / 2;
        bool goal_position_is_behind = goal_pose.pose.position.x < 0.0;

        // robot should move forward and goal is already behind, or robot should move backward and goal is already in front
        if(goal_direction_is_forward == goal_position_is_behind) {
