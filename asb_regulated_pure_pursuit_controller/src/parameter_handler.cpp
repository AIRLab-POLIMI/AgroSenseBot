...
ParameterHandler::ParameterHandler(rclcpp_lifecycle::LifecycleNode::SharedPtr node, std::string &plugin_name, rclcpp::Logger &logger, const double costmap_size_x) {
    ...
    declare_parameter_if_not_declared(node, plugin_name_ + ".min_departure_linear_velocity", rclcpp::ParameterValue(0.05));
    ...
    declare_parameter_if_not_declared(node, plugin_name_ + ".departure_velocity_scaling_dist", rclcpp::ParameterValue(0.6));
    ...
    declare_parameter_if_not_declared(node, plugin_name_ + ".max_robot_path_dist", rclcpp::ParameterValue(costmap_size_x));
    declare_parameter_if_not_declared(node, plugin_name_ + ".max_robot_pose_search_dist", rclcpp::ParameterValue(costmap_size_x / 2.0));
    ...
    node->get_parameter(plugin_name_ + ".min_departure_linear_velocity", params_.min_departure_linear_velocity);
    ...
    node->get_parameter(plugin_name_ + ".departure_velocity_scaling_dist", params_.departure_velocity_scaling_dist);
    if (params_.departure_velocity_scaling_dist > costmap_size_x / 2.0) {
        RCLCPP_WARN(logger_, "departure_velocity_scaling_dist is larger than forward costmap extent, leading to permanent slowdown");
    }
    ...
    node->get_parameter(plugin_name_ + ".inflation_cost_scaling_factor", params_.inflation_cost_scaling_factor);
    ...
    node->get_parameter(plugin_name_ + ".max_robot_path_dist", params_.max_robot_path_dist);
    node->get_parameter(plugin_name_ + ".max_robot_pose_search_dist", params_.max_robot_pose_search_dist);
    if (params_.max_robot_pose_search_dist < 0.0) {
        RCLCPP_WARN(logger_, "Max robot search distance is negative, setting to max to search"
    ...
    node->get_parameter(plugin_name_ + ".use_collision_detection", params_.use_collision_detection);
    ...
    dyn_params_handler_ = node->add_on_set_parameters_callback(std::bind(&ParameterHandler::dynamicParametersCallback, this, std::placeholders::_1));
}

rcl_interfaces::msg::SetParametersResult ParameterHandler::dynamicParametersCallback(std::vector<rclcpp::Parameter> parameters) {
    ...
    for (const auto& parameter: parameters) {
        ...
        if (type == ParameterType::PARAMETER_DOUBLE) {
            ...
            } else if (name == plugin_name_ + ".adaptive_lookahead_path_distance_margin") {
            ...
            } else if (name == plugin_name_ + ".min_departure_linear_velocity") {
                params_.min_departure_linear_velocity = parameter.as_double();
            ...
            } else if (name == plugin_name_ + ".max_robot_path_dist") {
                params_.max_robot_path_dist = parameter.as_double();
            }
            ...
            if (name == plugin_name_ + ".use_velocity_scaled_lookahead_dist") {
                ...
                if (parameter.as_bool() && params_.allow_reversing) {
                    RCLCPP_WARN(logger_, "Both use_rotate_to_heading and allow_reversing "
