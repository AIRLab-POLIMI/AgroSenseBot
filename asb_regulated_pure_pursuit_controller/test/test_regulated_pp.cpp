...
class BasicAPIRPP : public asb_regulated_pure_pursuit_controller::RegulatedPurePursuitController
{
public:
  BasicAPIRPP()
...
TEST(RegulatedPurePursuitTest, testDynamicParameter)
{
  ...
  costmap->on_configure(rclcpp_lifecycle::State());
  auto ctrl =
    std::make_unique<asb_regulated_pure_pursuit_controller::RegulatedPurePursuitController>();
  auto tf = std::make_shared<tf2_ros::Buffer>(node->get_clock());
